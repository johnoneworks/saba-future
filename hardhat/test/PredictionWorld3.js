const { time, loadFixture } = require("@nomicfoundation/hardhat-network-helpers");

const { expect } = require("chai");


describe("PredictionWorld3", function () {

  async function deploySurePredictionWorldFixture() {
    const [owner, other1] = await ethers.getSigners();
    const SureToken = await ethers.getContractFactory("SureToken3");
    const sureToken = await SureToken.deploy();
    const decimals = await sureToken.decimals()

    const PredictionWorld = await ethers.getContractFactory("PredictionWorld3");
    const predictionWorld = await PredictionWorld.deploy(sureToken.address);

    return { predictionWorld, sureToken, owner, other1, decimals };
  }

  describe("Main Flow", function () {

    it("Should set the right sure address", async function () {
      const { predictionWorld, sureToken } = await loadFixture(deploySurePredictionWorldFixture);
      expect(await predictionWorld.sureToken()).to.equal(sureToken.address);
    });

    it("Should create a prediction when provided with valid params", async function () {
      const { predictionWorld } = await loadFixture(deploySurePredictionWorldFixture);

      // Prepare the test data.
      const question = "Can I win the lottery?";
      const creatorImageHash = "data:image/jpeg;base64,";
      const description = "Strong Luck";
      const resolverUrl = "https://resolverUrl";
      const endTimestamp = Date.now();

      // Check the # markets
      expect(await predictionWorld.totalMarkets()).is.equal(0)

      // Create a market
      await predictionWorld.createMarket(question, creatorImageHash, description, resolverUrl, endTimestamp);

      // Check if the market is created.
      expect(await predictionWorld.totalMarkets()).is.equal(1)

      // Check the content of the market.
      const market = await predictionWorld.markets(0);
      expect(market.question).to.equal(question);
      expect(market.creatorImageHash).to.equal(creatorImageHash);
      expect(market.description).to.equal(description);
      expect(market.resolverUrl).to.equal(resolverUrl);
      expect(market.endTimestamp).to.equal(endTimestamp);
    });

    it("Should create a bet with valid params", async function () {
      const { predictionWorld, sureToken, owner, decimals } = await loadFixture(deploySurePredictionWorldFixture);

      // Prepare the test data.
      const question = "Can I win the lottery?";
      const creatorImageHash = "data:image/jpeg;base64,";
      const description = "Strong Luck";
      const resolverUrl = "https://resolverUrl";
      const endTimestamp = Date.now();

      // Check the # markets
      expect(await predictionWorld.totalMarkets()).is.equal(0)

      // Create a market
      await predictionWorld.createMarket(question, creatorImageHash, description, resolverUrl, endTimestamp);

      // Check if the market is created.
      expect(await predictionWorld.totalMarkets()).is.equal(1)

      // Check the content of the market.
      const market = await predictionWorld.markets(0);
      expect(market.question).to.equal(question);
      expect(market.creatorImageHash).to.equal(creatorImageHash);
      expect(market.description).to.equal(description);
      expect(market.resolverUrl).to.equal(resolverUrl);
      expect(market.endTimestamp).to.equal(endTimestamp);

      // Prepare data to bet
      const marketId = 0;
      const marketValue = ethers.BigNumber.from(2);

      // Save the original balance
      const ownerBalance = await sureToken.balanceOf(owner.address);
      const leftBalance = ownerBalance.sub(marketValue);
      
      const ownerContract = sureToken.connect(owner);
      await ownerContract.approve(predictionWorld.address, ethers.utils.parseUnits(marketValue.toString(), decimals));

      // Execute to bet
      await predictionWorld.addYesBet(marketId, marketValue);

      // Check the contract's balance
      expect(await sureToken.balanceOf(predictionWorld.address)).to.equal(marketValue);

      // Check the owner's balance
      expect(await sureToken.balanceOf(owner.address)).to.equal(leftBalance);

      // Check the amount in the market
      const market2 = await predictionWorld.markets(0);
      expect(market2.totalYesAmount).to.equal(marketValue);
      expect(market2.totalAmount).to.equal(marketValue);
      expect(market2.totalNoAmount).to.equal(0);

      // Check the yes bet
      const [yesBets, noBets] = await predictionWorld.getBets(0);
      expect(yesBets[0].user).to.equal(owner.address);
      expect(yesBets[0].amount).to.equal(marketValue);
      expect(noBets.length).to.equal(0);
    });
  });
});