...
describe("PredictionWorld6", function () {
    ...
    async function deploySurePredictionWorldFixture() {
        ...
        const PredictionWorld = await ethers.getContractFactory("PredictionWorld6");
    ...
    describe("Test Markets", function () {
        ...
        it("Should create/get markets with/without test items", async function () {
            ...
            const fetchResult = await predictionWorld.fetchMarkets(0, 10, true);
            const markets = fetchResult[0];
            expect(markets.length).is.equal(2);

            // Get 1 markets when [withTest==false]
            const fetchResult2 = await predictionWorld.fetchMarkets(0, 10, false);
    ...
    describe("Fetch Markets with pagination", function () {
        ...
        it("Should return correct data with divisible size", async function () {
            const { predictionWorld } = await loadFixture(deploySurePredictionWorldFixture);
            ...
            //  => Get first page.
            const pageSize = 10;
            const fetchResult = await predictionWorld.fetchMarkets(0, pageSize, true);
            const markets = fetchResult[0];
            expect(markets.length).is.equal(10);
            //  => Get second page.
            const nextCursor = fetchResult[1];
            const fetchResult2 = await predictionWorld.fetchMarkets(nextCursor, pageSize, true);
            const markets2 = fetchResult2[0];
            expect(markets2.length).is.equal(10);

            // Get 10 markets when [withTest==false]
            //  => Get first page.
            const fetchResult3 = await predictionWorld.fetchMarkets(0, pageSize, false);
            const markets3 = fetchResult3[0];
            expect(markets3.length).is.equal(10);
            //  => Get second page.
            const nextCursor4 = fetchResult3[1];
            const fetchResult4 = await predictionWorld.fetchMarkets(nextCursor4, pageSize, false);
            const markets4 = fetchResult4[0];
            expect(markets4.length).is.equal(0);
        });

        it("Should return correct data with NOT divisible size", async function () {
            const { predictionWorld } = await loadFixture(deploySurePredictionWorldFixture);
            ...
            const fetchResult3 = await predictionWorld.fetchMarkets(nextCursor2, pageSize, true);
            const markets3 = fetchResult3[0];
            expect(markets3.length).is.equal(6);

            // Get 10 markets when [withTest==false]
            //  => Get first page.
            const fetchResult4 = await predictionWorld.fetchMarkets(0, pageSize, false);
            const markets4 = fetchResult4[0];
            expect(markets4.length).is.equal(10);
            //  => Get second page.
            const nextCursor4 = fetchResult4[1];
            const fetchResult5 = await predictionWorld.fetchMarkets(nextCursor4, pageSize, false);
